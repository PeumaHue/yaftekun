...
class Jugador extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->subeimagen_aptomedico = false;
		$this->subeimagen_aptomedico_conf_['upload_path'] = './images/certificados_medicos/';
		$this->subeimagen_aptomedico_conf_['allowed_types'] = 'gif|jpg|png';
	...
	public function alta()
	{
		...
		if ($this->input->method()=='post')
		{
			...
			else
			{
				...
				{
					$this->load->library('upload', $this->subeimagen_aptomedico_conf_);
					$this->upload->initialize($this->subeimagen_aptomedico_conf_);
					if (!$this->upload->do_upload('nombre_archivo_apto_medico'))
					{
						$this->variables['mensaje'] = $this->upload->display_errors();
					}
					else
					{
						$this->subeimagen_aptomedico_nombre_archivo_final = $this->upload->data('file_name');
					}
				}
	...
	private function _obtener_post($id_participante=NULL)
	{
		...
		if ($this->subeimagen_aptomedico)
		{
			$participante->nombre_archivo_apto_medico 	= $this->subeimagen_aptomedico_nombre_archivo_final;
		}
		else
		{
			$participante->nombre_archivo_apto_medico 	= $this->input->post('imagen_original_aptomedico');
		}
		return $participante;
	}
	...
	private function _setear_campos()
	{
		...
		$this->datos_formulario->imagen_original_aptomedico = isset($this->datos_formulario->imagen_original_aptomedico) ? $this->datos_formulario->imagen_original_aptomedico : '';
